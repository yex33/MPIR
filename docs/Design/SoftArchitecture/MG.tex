...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[%
    ...
    \draw (-5,-4) node[block] (input) {Read Matrix Module (\mref{M:ops})};
    \draw (0,-4) node[block] (solve) {Iterative Solves Module (\mref{M:concepts})};
    \draw (7,-2.5) node[block] (output) {Solver Output Module (\mref{M:solve})};
