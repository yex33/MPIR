...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[%
    ...
    \draw (0,0) node[block] (example) {Example Program Module (\mref{M:example})};
    \draw (-5,-4) node[block] (input) {Read Matrix Module (\mref{M:input})};
    \draw (0,-4) node[block] (solve) {Iterative Solves Module (\mref{M:solve})};
    \draw (7,-2.5) node[block] (output) {Solver Output Module (\mref{M:output})};
    \draw (7,-5.5) node[block] (factor) {Matrix Factorization Module (\mref{M:factor})};
