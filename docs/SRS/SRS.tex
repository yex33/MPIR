...
\item User Responsibilities:
  ...
  \item Provide a/multiple vector(s) \(\vec{b}: \mathbb{R}^n\) to be solved for.
...
\item \progname{} Responsibilities:
  \begin{itemize}
  \item Numerically solve \(\matr{A}\vec{x} = \vec{b}\) for \(\vec{x}\).
  \end{itemize}
\end{itemize}
...
% #5 description
{ For any permutation \(\matr{P}\) of some symmetric quasi-definite matrix
  \(\matr{K}\), there is a diagonal \(\matr{D}\) and a unit
...
{Krylov subspace}
% #4 equation
{ \(\mathcal{K}_m(\matr{A}, \vec{r}_0) = \spann{\vec{r}_0, \matr{A}\vec{r}_0,
...
{ \(\matr{A}\) is any given real, \(n \times n\) matrix. In the context of
  ...
  \matr{A}\vec{x}_0\) is the initial residual. \(\mathcal{K}_m(\matr{A},
...
{ Intuitively, a matrix \(\matr{A}\) describes some kind of linear
  ...
  taming \(\matr{A}\) down to some extent. We can then work with a tamed down,
  better conditioned version of \(\matr{A}\) and solve the system from there. }
% #7 Source
    ...
    \begin{algorithm}[H]
      ...
      \begin{algorithmic}[1]
        \For{\(m \gets 1, 2, \dots\), the \(m\)th iteration}
          \State \(\vec{r}_m \gets \vec{b} - \matr{A}\vec{x}_m\) \Comment{Compute the residuals (in
            high precision)}\label{algo:ir:residual}
