...
The goal \gsref{GS:Axb} is solved by \iref{IM:IM}.
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R:MP}:] The solver should offer
  customizability on the arithmetic precision used to perform matrix
  factorization, iterative refinement, and residual evaluation.

\item[R\refstepcounter{reqnum}\thereqnum \label{R:ex}:] The accompanying package
  should provide one or many example programs to demonstrate the process of
  preparing the inputs for the solver, consuming the solver APIs, and observing
