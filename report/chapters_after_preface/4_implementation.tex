...
The QD package provides double-double (\texttt{dd\_real}) and quad-double
  ...
  \begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
      ...
      \For{\(k = 1, 2, \dots, n_{\text{inner}}\)}
        ...
        \State \(\beta_2 = \norm{\vec{w}}_2\)
        \If{\(\beta_1 + 0.001 \times \beta_2 = \beta_1\)} \Comment{Brown/Hindmarsh condition}
      ...
      \EndFor
      \State \(\bar{\matr{H}}_k \gets \{h_{i,j}\}_{1 \le i,j \le k}\) \Comment{Form upper
      Hessenberg matrix}
...
\begin{singlespace}
  \begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
      ...
      \For{\(i = 1, 2, \dots, k\)}
        \State \(S \gets S^2\) \Comment{Compute sparsity pattern symbolically}
      \EndFor
      \State \(u_{i,j} = a_{i, j},\quad \matr{U} \gets \{u_{i,j}\}\) \Comment{Copy
        \(\matr{A}\) as initial guesses}
      \State \(l_{i,j} = a_{i, j},\quad \matr{L} \gets \{l_{i,j}\}\)
      \For{\(\text{sweep} = 1, 2, \dots n_{\text{sweep}}\) or until convergence}
